...
void SaliencyDetection(string srcPath,string dstPath);


int main(){


	char endfile[256] = ".png";


	for(int i = 1; i <10;i++){
		char srcPath[256] = "refimages/";

			char dstPath[256] = "dstimages/";

			char strdstfilename[256] = "Saliency";
		...
		char filename[256] = "00";

			strcat(filename,stri);
			strcat(strdstfilename,filename);

			//strdstfilename = strdstfilename +filename;

			//srcPath = srcPath+filename+endfile;
			strcat(filename,endfile);
			strcat(srcPath,filename);


			//dstPath = dstPath+strdstfilename+endfile;
			strcat(strdstfilename,endfile);
			strcat(dstPath,strdstfilename);

			printf("srcPath= %s\n",srcPath);
			printf("dstPath= %s\n",dstPath);

			SaliencyDetection(srcPath,dstPath);
	}
...
void SaliencyDetection(string srcPath,string dstPath)
{
   // string imagePath = "refimage/041.png";
    ...
    src = cvLoadImage(srcPath.c_str(),0);
   ...
   // cvShowImage("Saliency",Saliency);
    ...
    cvSaveImage(dstPath.c_str(),Saliency);
